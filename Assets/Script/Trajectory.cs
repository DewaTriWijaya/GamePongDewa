...
public class Trajectory : MonoBehaviour
{
    ...
    void Update()
    {
        // Inisiasi status pantulan lintasan bertumbukan dengan objek tertentu
        bool drawBallAtCollision = true;

        // Titik tumbukan yang digeser, untuk menggambar ballAtCollision
        Vector2 offsetHitPoint = new Vector2();

        // Tentukan titik tumbukan dengan deteksi pergerakn lingkaran
        ...
        foreach(RaycastHit2D circleCastHit2D in circleCastHit2DArray)
        {
            ...
            if (circleCastHit2D.collider != null && circleCastHit2D.collider.GetComponent<BallControl>() == null)
            {
                ...
                // yaitu sebuah titik yang di-offset dari titik tumbukan berdasarkan vektor normal titik tersebut sebesar jari2 bola
                ...
                // Gambar garis lintasan dari titik tengah bola saat ke titik tengah bola pada saat bertumbukan
                DottedLine.DottedLine.Instance.DrawDottedLine(ball.transform.position, offsetHitPoint);

                // Kalau bukan sidewall, gambar pantulannya
                if (circleCastHit2D.collider.GetComponent<SideWell>() == null)
                {
